...
import { NftIdentifier, NftTokenOwner, TokenInfo } from '@/01_types/nft';
...
import { unwrapOption, unwrapOptionMap, wrapOptionMap } from '@/02_common/types/options';
...
import {
    ...
    OgyTokenActive,
    OgyTokenHistory,
    parseOgyAuctionToNftListing_243d1642,
    parseOgyCollectionInfo_243d1642,
    unwrapActiveRecords,
    unwrapHistoryRecords,
} from '..';
import idlFactory from './ogy_243d1642.did';
import _SERVICE, {
    AuctionStateShared,
    BidResponse,
    CandyShared,
...
export const queryCollectionInfoByOgy = async (
    ...
    >(r, parseOgyCollectionInfo_243d1642, throwsBy(`can not query ogy info: ${collection}`));
};
            ...
            owner: unwrapVariant4Map(
                ...
                [
                    ...
                    (s: { owner: Principal; sub_account: [] | [Uint8Array | number[]] }) =>
                        principal2account(
                            principal2string(s.owner),
                            unwrapOptionMap(s.sub_account, (s) => Array.from(s)),
                        ),
                ],
            ),
            raw: {
...
export const queryTokenListingByOgy = async (
    ...
    return list.map((d, index) => {
        ...
        listing_data.listing = unwrapVariant1Map<AuctionStateShared, NftListing>(
            current_sale.sale_type,
            [
                'auction',
                (a) =>
                    parseOgyAuctionToNftListing_243d1642(current_sale.sale_id, a, yumi_ogy_broker),
            ],
        );
        return listing_data;
    });
