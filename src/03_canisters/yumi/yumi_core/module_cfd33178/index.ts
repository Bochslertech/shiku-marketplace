...
import { parseCollectionStandard_6f12cd52, unwrapCollectionLinks } from '../../../yumi/types';
import idlFactory from './core_cfd33178.did';
import _SERVICE, {
...
export const parseCollectionInfo = (info: CollectionInfo__1): CollectionInfo => ({
    ...
    royalties: exponentNumber(
        bigint2string(info.royalties.fee),
        -Number(bigint2string(info.royalties.precision)) + 2,
    ),
    category: unwrapOption(info.category),
    standard: parseCollectionStandard_6f12cd52(info.standard),
...
const parseListingFee = (fee: CandidFee): ListingFee => {
    return {
        platform: exponentNumber(
            bigint2string(fee.platform.rate),
            -Number(bigint2string(fee.platform.precision)),
        ),
        royalties: exponentNumber(
            bigint2string(fee.royalties.rate),
            -Number(bigint2string(fee.royalties.precision)),
        ),
    };
};
