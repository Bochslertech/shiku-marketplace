...
function ProfileCreated({
    ...
    // console.debug('profile created', principal, created.length);
    ...
    useEffect(() => {
        once_load.once([created], () => {
            setList(undefined);
            setLoading(true);
            loadNftCardsByStoredRemote(collectionDataList, created, wrappedSetList).finally(() => {
                setLoading(false);
                // 加载完成后再确认一次所有权
                getNftCardsByRemote(collectionDataList, created).then(wrappedSetList);
            });
        });
    }, [created]);
    ...
    const silenceRefreshList = useCallback(() => {
        ...
        getNftCardsByRemote(collectionDataList, created).then(wrappedSetList);
    }, [showed, created, collectionDataList]);
